...
(function (module) {
    ...
    Article.numWordsAll = () => {
        return Article.all.map(articleObject => articleObject.body).reduce((acc, body) => acc + body);
    };
    ...
    Article.allAuthors = () => {
        return Article.all.map(articleObject => articleObject.author).reduce((acc, author) => acc + author);
    };
